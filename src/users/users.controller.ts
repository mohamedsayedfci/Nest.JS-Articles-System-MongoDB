...
export class UsersController {
  constructor(
    ...
    private readonly emailVerificationService: EmailVerificationService, // private readonly caslAbilityFactory: CaslAbilityFactory
  ) {}
  ...
  async createNewUser(@Body(RegisterPipe) userData: CreateUsersDto) {
    ...
    if (data) {
      ...
      const storeEmailCode =
        await this.emailVerificationService.createNewEmailVerification(
          registerCode,
          EXPIRE_CODE_TIME,
          data._id,
        );
  ...
  async activateEmail(@Body() emailActivateData: CreateEmailActivateDto) {
    ...
    if (!data) {
      throw new BadRequestException(
        `Not send code to user with id = ${emailActivateData.user}`,
      );
