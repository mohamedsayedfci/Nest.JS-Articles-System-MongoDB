...
export class ArticlesController {
    ...
    @UseGuards(PoliciesGuard)
    @UseGuards(JwtAuthGuard)
    async getArticleById(@Param('id', ParseUUIDPipe) id: string, @Request() req) {

        let data = await this.articlesService.getArticleById(id)
        if (!data) throw new NotFoundException(`No articles with this _id = ${id}`)

        let comments= (await this.commentsService.getAllCommentsOnArticlesDeatils(id))
        data = (data as any).toObject();
        let isLiked=await this.likesService.checkLikeArticle(req.user._id,id);
        data['is_liked']=isLiked?true:false;;
